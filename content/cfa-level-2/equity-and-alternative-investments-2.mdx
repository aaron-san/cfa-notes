---
title: "CFA&reg; Level 2: Alternative Investments (part 2)"
...
backUrl: "blog/cfa-level-2/alternative-investments-1"
---
...
<Math>
  {"\\mathrm {\\small {POST =  \\frac{\\text {Exit Value}} {(1 + r)^t}}}"}
</Math>
...
<Math>
  {
    "\\mathrm {\\small {\\text {Exit Value} = \\text{Earnings Multiple} \\times \\text{Estimate of terminal earnings}}}"
  }
</Math>

<Math>
  {
    "\\mathrm {\\small {\\text {Exit Value} = \\text{PV(expected future cash flows)}}}"
  }
</Math>
...
<Math>{"mathrm {small {\text {PRE} = \text{POST - Investment}}}"}</Math>

- Step 3: Determine the ownership fraction for the investor (f)

<Math>{"\\mathrm {\\small {f = \\frac{\\text {Investment}} {POST}}}"}</Math>
