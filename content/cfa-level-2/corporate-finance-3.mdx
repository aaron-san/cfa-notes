...
- Default risk and expected bankruptcy costs, taxes, and agency costs all contribute to the rate of change of firm value with changing financing mix
- There is a tax benefit of debt (tax deductible interest expense)
...
- The cost of equity increases linearly as a company increases its proportion of debt financing
  <Math>{"$r_e = r_0 + \\dfrac{D}{E}(r_0 - r_d)$"}</Math>
...
### MM Proposition II (With Taxes): WACC is Minimized at 100% Debt
  ...
  <Math>{"$r_e = r_0 + \\dfrac{D}{E}(r_0 - r_d)(1-t)$"}</Math>
