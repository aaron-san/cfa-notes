...
```math
\mathrm {\small {SF = \frac{E(R_{p}) - R_{L}} {\sigma _{p}}}}
```
...
```math
\mathrm {\small \frac{S_{t+1}} {S_{t}} = 1 + R_{t, t+1}}
```

## Continuously Compounded Return

`math \mathrm {\small r_{t, t+1} = ln(\frac{S_{t+1}} {S_{t}}) = ln(1 + R_{t, t+1})}`
...
```math
\mathrm {\small Value_{T} = Max(0, S_{T} - \bar{S})}
```
