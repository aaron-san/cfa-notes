...
<Math>{"$R_{BD} = \\dfrac{D}{F} \\times \\dfrac{360}{t}$"}</Math>
...
<Math>{"HPY = $\\dfrac{ P_{1} - P_{0} + D_{1}}{P_{0}}$"}</Math>
...
<Math>
  {
    "$r_{mm} = \\dfrac{ 360 \\times (r_{BD})} { 360 - (r_{BD})t} = r_{BD} \\dfrac{F}{P_0} = HPY \\dfrac{360} {t}$"
  }
</Math>
